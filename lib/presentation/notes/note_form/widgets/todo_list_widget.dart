...
import 'package:brokerage/presentation/notes/note_form/misc/build_context_x.dart';
    ...
    return MultiBlocListener(
      listeners: [
        ...
        BlocListener<NoteFormBloc, NoteFormState>(
          ...
          listenWhen: (previousState, state) =>
              previousState.note.todos.length != 0 &&
              previousState.note.todos.isFull != state.note.todos.isFull,
          listener: (context, state) {
            if (state.note.todos.isFull) {
              FlushbarHelper.createAction(
                message: '希望更长列表？激活商业版 🤩',
                button: FlatButton(
                  onPressed: () {
                    print('买买买! 🤑');
                  },
